...
class Simplifier:
    ...
    def appositive_phrases(self, dep_dict, words, root, dep_root, ant):
        ...
        if 'nsubj' in dep_root:
            ...
            final_root = {}
            self.build(root, dep_root, dep_dict, [s[0].lower() for s in words], final_root)
            final_appos = {}
            self.build(mod, deps_mod, dep_dict, [s[0].lower() for s in words], final_appos)
            final_subj = {}
            self.build(subj, deps_subj, dep_dict, [s[0].lower() for s in words], final_subj)
        ...
        for s in sentences:
            ...
            if 0 in dep_dict:
                ...
                if root in dep_dict:
                    ...
                    flag_appos, res = self.appositive_phrases(dep_dict, words, root, dep_root, sentences) 
                    if flag_appos:
                        result += res
                        continue
                    ...
                    if flag_rc:
                        if 'nsubj' in dep_root:
                            flag, res = self.relative_clauses(dep_dict, tokens, root, dep_root, 'nsubj', sentences)
                            if flag:
                                result += res
                                continue
                        elif 'dobj' in dep_root:
                            flag, res = self.relative_clauses(dep_dict, tokens, root, dep_root, 'dobj', sentences)
                            if flag:
                                result += res
                                continue
            result.append(s)
